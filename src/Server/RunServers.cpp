...
int RunServers::bindSockets(Server &server) {
    ...
    logMessage(SINFO, "Server listening on http://" +
                          server.serverConf.DefaultServerConfig.Host + ":" +
                          server.serverConf.DefaultServerConfig.Port);
    std::cout << BOLD FG_BLUE "Server socket created with descriptor: "
              << server.socketServer << RESET_ALL << std::endl;
